...
image: ../figures/02b_actor_critic_schematic.png
---

> *Policy gradient methods, from scratch — 2 of 4.*

[Part 2a](02a-reinforce-and-policy-gradient.qmd) ended with a question that this post answers. We saw that REINFORCE's gradient has high variance, and that subtracting a baseline reduces it. The best baseline is $V^{\pi_\theta}$ — the *current* policy's value function. But we don't have it. The running-mean baseline was a crude substitute that helped a lot but not enough.
...
- $\lambda = 0$: $\hat A_t = \delta_t$, the TD(0) estimator.
- $\lambda = 1$: $\hat A_t = \sum_k \gamma^k \delta_{t+k}$, which by telescoping equals $G_t - V_\phi(s_t)$ — Monte Carlo.
- Intermediate $\lambda$: a smooth blend.
...
- At $\lambda = 0$, variance is small but bias is large and negative (the critic underestimates).
- At $\lambda = 1$, bias is large and positive (the bootstrap term carries no critic information at all; bias comes from the single empirical sample).
- The curve passes through zero bias around $\lambda \in [0.6, 0.8]$.
...
Now we tie it together. **Advantage Actor-Critic** (A2C) is the bread-and-butter actor-critic algorithm. Each episode:
...
5. **Critic update**: for each $t$, $V_\phi(s_t) \leftarrow V_\phi(s_t) + \alpha_\phi (\hat A_t + V_\phi(s_t) - V_\phi(s_t))$. The target $\hat A_t + V_\phi(s_t)$ is the bootstrapped return; we regress $V_\phi(s_t)$ toward it.

In our tabular NumPy version, both updates are explicit gradient steps. For neural-network policies, both would use autodiff on appropriate loss functions:

$$
\mathcal{L}_{\text{actor}} = -\sum_t \hat A_t \log \pi_\theta(a_t \mid s_t),
\qquad
\mathcal{L}_{\text{critic}} = \sum_t \frac{1}{2}\big(\hat A_t + V_\phi(s_t) - V_\phi(s_t)^\text{detached}\big)^2.
$$

The `detached` trick is essential: we don't want the critic loss to backprop through the advantage computation, because that would couple the gradient of $V_\phi$ inappropriately.
...
- **PPO** (Schulman et al. 2017): A2C+GAE plus a clipping mechanism on the policy ratio to prevent overly-large updates. The next post (2c) builds it.
...
- **GRPO** (Shao et al. 2024): A2C with a group-relative baseline that bypasses the critic entirely — particularly effective for LLM reasoning.
