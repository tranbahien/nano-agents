...
image: ../figures/02d_rlhf_pipeline.png
---
...
Critically, this objective has a closed-form solution. Setting the gradient to zero, the optimal policy is
...
where $Z(x) = \sum_y \pi_{\text{ref}}(y \mid x) \exp(r(y, x) / \beta)$ is the per-prompt partition function. The policy *tilts* the reference distribution toward high-reward outputs, with strength controlled by $1/\beta$.

This is a familiar object — it's the **Gibbs posterior** with the reference as prior and reward as log-likelihood. It's also why RLHF can be viewed as a kind of approximate Bayesian inference over policies.
