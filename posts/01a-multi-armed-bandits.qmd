...
![UCB1 over time. The orange exploration bonus shrinks as each arm's pull count grows; eventually the indices reflect empirical means and the best arm wins. *(Code to reproduce: [`experiments/01a_ucb_bonus_shrinking.py`](https://github.com/tranbahien/nano-agents/blob/main/experiments/01a_ucb_bonus_shrinking.py)).*](../figures/01a_ucb_bonus_shrinking.png)
...
![Beta posteriors over time. By t=5000, the algorithm has identified Arm 3 (μ=0.7) as best. Arms 2 and 3 — the close competitors — are sampled most because they're hardest to distinguish. Arm 1 is largely ignored once it's clearly suboptimal, so its posterior stays wide. *(Code to reproduce: [`experiments/01a_beta_posterior_evolution.py`](https://github.com/tranbahien/nano-agents/blob/main/experiments/01a_beta_posterior_evolution.py)).*](../figures/01a_beta_posterior_evolution.png)
...
![Probability matching. Each pair of bars shows analytical $P(\mu_k = \max)$ vs the empirical frequency with which Thompson Sampling picks arm $k$ from 100,000 independent samples. They match to three decimal places. *(Code to reproduce: [`experiments/01a_probability_matching.py`](https://github.com/tranbahien/nano-agents/blob/main/experiments/01a_probability_matching.py)).*](../figures/01a_probability_matching.png)
...
![Cumulative fraction of pulls allocated to each arm under each algorithm (averaged over 30 runs). The brightest band on top is the best arm (μ=0.8). ε-greedy keeps ~20% exploration forever. UCB1 explores aggressively then narrows but stays diffuse. Thompson concentrates fastest. *(Code to reproduce: [`experiments/01a_arm_pull_distribution.py`](https://github.com/tranbahien/nano-agents/blob/main/experiments/01a_arm_pull_distribution.py)).*](../figures/01a_arm_pull_distribution.png)
...
![Cumulative regret of three classical bandit algorithms on a 5-arm Bernoulli problem. We will derive each of these from scratch and end with this exact figure. *(Code to reproduce: [`experiments/01a_regret_curves.py`](https://github.com/tranbahien/nano-agents/blob/main/experiments/01a_regret_curves.py)).*](../figures/01a_regret_curves.png)
...
### 9.1 Gap sensitivity
![Gap sensitivity: the same three algorithms on an easy bandit (left) and a hard one (right). Note the very different y-axis scales — the hard bandit's regret is ~5× smaller in absolute terms. *(Code to reproduce: [`experiments/01a_sol1_gap_sensitivity.py`](https://github.com/tranbahien/nano-agents/blob/main/experiments/01a_sol1_gap_sensitivity.py)).*](../figures/01a_sol1_gap_sensitivity.png)
...
![Non-stationary bandit. Top: cumulative regret over 8000 steps as the true arm means drift sinusoidally (bottom). UCB1 — counter-intuitively — handles drift best of the three. *(Code to reproduce: [`experiments/01a_ucb_bonus_shrinking.py`](https://github.com/tranbahien/nano-agents/blob/main/experiments/01a_sol2_nonstationary.py)).*](../figures/01a_sol2_nonstationary.png)
...
![Gaussian bandit with means [0.0, 0.5, 1.0, 1.2, 1.5] and σ=1.0. Gaussian UCB and NIG-Thompson both achieve sublinear regret; ε-greedy with the same configuration is markedly worse. *(Code to reproduce: [`experiments/01a_sol3_gaussian_rewards.py`](https://github.com/tranbahien/nano-agents/blob/main/experiments/01a_sol3_gaussian_rewards.py)).*](../figures/01a_sol3_gaussian_rewards.png)
...
![Prior sensitivity. Strong miscalibrated priors (Beta(100,100), red) force linear regret because the algorithm can't update beliefs away from 0.5 fast enough. Weak priors and uniform priors are essentially equivalent. *(Code to reproduce: [`experiments/01a_sol4_prior_sensitivity.py`](https://github.com/tranbahien/nano-agents/blob/main/experiments/01a_sol4_prior_sensitivity.py)).*](../figures/01a_sol4_prior_sensitivity.png)
