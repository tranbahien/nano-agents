...
image: ../figures/02a_pg_schematic.png
---

> *Policy gradient methods, from scratch — 1 of 4.*

Topic 1 built the value-based foundation: learn $Q^\star$, then act greedily. That worked beautifully on small finite MDPs. It also has a fatal limitation for everything modern: **it doesn't scale to neural-network policies on continuous actions.** Argmax over a continuous action space is hard; argmax over a neural network's output is intractable; the policy is *implicit* in $Q$ rather than directly expressed.
...
What's the best $b(s)$? The choice that minimizes the variance of the gradient estimator is

$$
b^\star(s) \;=\; \mathbb{E}_{\pi_\theta}[G_t \mid s_t = s] \;=\; V^{\pi_\theta}(s).
$$

The optimal baseline is the **current policy's value function**. This is intuitive: $A_t = G_t - V^{\pi_\theta}(s_t)$ measures how much *better* the actual return was than what the agent expects on average from state $s_t$. Subtracting the expected return centers the advantage around zero.
...
In modern RLHF, the policy is an LLM, and the "reward" comes from a learned reward model trained on human preferences. The gradient is

$$
\nabla_\theta J(\theta) \;=\; \mathbb{E}_{x \sim \mathcal{D}, \, y \sim \pi_\theta(\cdot \mid x)}\!\left[ r_\phi(x, y) \, \nabla_\theta \log \pi_\theta(y \mid x) \right]
$$

— exactly the REINFORCE estimator, with prompts $x$ as states and responses $y$ as actions. The only addition in practice is a KL penalty against a reference policy to prevent collapse to degenerate distributions.
