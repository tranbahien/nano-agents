...
image: ../figures/02c_clipped_surrogate.png
---
...
The "drift too far" is the central concern. If $\pi_{\text{new}}$ ever assigns probability ratio of, say, 100 to some action, the variance of the importance-sampled estimator explodes. The trust region is precisely about preventing this.
...
- Critics in LLM space are expensive to maintain.
- The within-prompt variance of rewards is what we want to reduce; the group baseline captures it well.
- Removing the critic eliminates a source of training instability (mis-calibrated critics).

GRPO is a striking example of how **knowing the structure of the algorithm lets you modify it**. The clipped surrogate is the load-bearing piece; the critic is a convenience.
